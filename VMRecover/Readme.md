...
24. Check back in the Serial Console, the log file you are tailing in the Serial Console will show the connect and eventual disconnect messages.  You can press _Control+C_ to interrupt.  As you can see at this point, the service is currently up and running, but the SSH connections are still failing, you can have this scenarios in real life where is not just one thing that could be happening.  In Lab2, we'll continue the troubleshooting to finish fixing current scenario. 
